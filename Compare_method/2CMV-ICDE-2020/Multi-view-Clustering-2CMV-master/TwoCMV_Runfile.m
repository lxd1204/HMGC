...
datasetName = {'3sources','10X_PBMC_4271n_12206d_8c_uni_Bs',...
    'prokaryotic','ORL_Xs','BBCSport_Xs','cornell_2v_Xs','wisconsin_2v_Xs','texas_2v_Xs','2V_BDGP','mnist4'};
...
%% initialise R is the data matrix 
    ...
    for iNMI = 1:size(NMIs_intra_new,1)
        ...
        Result_TCMV(idx,:) = [best.CA best.F best.P best.R best.nmi best.AR];
%         save([resultdir,char(datasetName(dataIndex)),'+',para_2,'_result.mat'],'Result_TCMV(idx,:)');
        idx = idx+1;
    end
    maxresult = max( Result_TCMV,[],1);
    fprintf('- Finish Proposed Method \n ');
    save([resultdir,char(datasetName(dataIndex)),'_summary_','_result.mat'],'maxresult');
end